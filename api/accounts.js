...
function getSessionFromReq(req) {
  return { vaultKey: getVaultKeyFromEvent(req), userId: getUserIdFromEvent(req) }
}
...
function sanitizeInput(body) {
  ...
  try {
    ...
    try { Secret.base32(cleanSecret) } catch { throw new Error('Invalid base32 secret') }
  }
...
export default async function handler(req, res) {
  const { vaultKey, userId } = getSessionFromReq(req)
  if (!vaultKey || !userId) {
  ...
  if (req.method === 'POST') {
    let input
    try { input = sanitizeInput(getBody(req)) }
