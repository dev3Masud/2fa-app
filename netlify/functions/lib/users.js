...
export function checkConfig() {
  ...
  if (pass.length < 8) {
    return { ok: false, message: 'ADMIN_PASS must be at least 8 characters' }
  }
  return { ok: true, username: user }
}

export async function ensureConfigOrError() {
  const cfg = checkConfig()
  if (!cfg.ok) return { error: errorResponse(500, cfg.message) }
  return { config: cfg }
}

export function buildWrappedKeyForUser(vaultKey, password) {
...
export async function bootstrapUser(username, password) {
  ...
  if (existing) return { error: errorResponse(409, 'User already exists') }
  const bcryptHash = await bcrypt.hash(password, BCRYPT_COST)
  const vaultKey = generateVaultKey()
  const { wrappedBlob, kekSalt, kekIterations } = buildWrappedKeyForUser(vaultKey, password)
...
export async function authenticateAndUnlock(username, password) {
  ...
  if (!user) return { error: errorResponse(401, 'Invalid credentials') }
  const ok = await bcrypt.compare(password, user.password_hash)
  if (!ok) return { error: errorResponse(401, 'Invalid credentials') }
  ...
  try {
    ...
    return { error: errorResponse(500, 'Vault unlock failed') }
  }
